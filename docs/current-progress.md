...
Kalshi Temps is currently a **local Seattle high-temperature market research tool**. It has a working FastAPI dashboard, SQLite persistence, public/manual collector foundations, scheduler one-shot status, official source and station metadata storage, deterministic validators, settlement replay, model adapter foundations, marine/cloud nowcast signals, historical backfill planning/runs, calibration records, paper-live tracking, DB ops checks, monitoring alerts/daily reports, optional env-token access gating, and precision dashboard/API integration. It is **not** a production-calibrated or production-scheduled trading system: it does not provide financial advice, guaranteed arbitrage, automated betting/order entry, compliance approval, permitted live Kalshi ingestion, licensed paid model feeds, actual satellite image processing, proven multi-week live operations, or production-grade auth/deployment. Settlement rules must be verified per market by the user before any research conclusion is treated as actionable context.
...
- `ops.py`: local SQLite/disk/access posture checks, DB health helpers, backup verification/prune/restore preflight support, and paper-live readiness summaries.
- `scheduler.py` / `monitoring.py`: one-shot collector scheduler/lock/status helpers plus monitoring alert and daily-report foundations.
...
- Calibration/backfill: `official_outcomes`, `prediction_snapshots`, `backfill_plans`, `backfill_runs`, `historical_bias`, `calibration_metrics`.
- Paper-live and audit/ops events: `paper_live_runs`, `paper_live_checklist_entries`, `paper_live_prediction_notes`, `paper_live_reconciliation_notes`, `paper_live_soak_metrics`, `alert_records`, `app_events`.
...
- Scheduler and monitoring: `run-scheduled-collectors`, `scheduler-status`, `run-monitoring-checks`, `list-alerts`, `resolve-alert`, `export-daily-report`.
- Backfill/calibration: `create-backfill-plan`, `run-backfill`, `record-prediction-snapshot`, `compute-calibration`, `calibration-report`.
- Paper-live and local operations: `start-paper-live-run`, `list-paper-live-runs`, `close-paper-live-run`, `record-paper-live-checklist`, `record-paper-live-prediction-note`, `record-paper-live-postmortem`, `record-paper-live-soak-metric`, `ops-status`, `db-check`, `backup-path`, `verify-backup`, `prune-backups`; shell scripts cover local run, demo seed, Tailscale access check, backup, and restore syntax.
...
- `/api/ops/status`, `/api/ops/db-health`, `/api/scheduler/status`, `/api/monitoring/alerts`, `/api/monitoring/daily-report`, `/api/paper-live/runs`, `/api/paper-live/runs/{run_id}`, `/api/paper-live/status`, `/api/observations`, `/api/sources`, `/api/official/observations`, `/api/fusion/summary`, `/api/market-snapshots`, `/api/model-runs`, `/api/model-spread`, `/api/model/adapters`, `/api/settlement/replays`, `/api/market/verification`, `/api/collector/health`, `/api/weather/features`, `/api/nowcast/signals`, `/api/calibration/summary`, and `/api/backfill/reports`.

The dashboard surfaces observations, source/provenance context, station metadata, official observations, daily high, model runs/spread/deltas, market snapshots, settlement replay summaries, market-rule verification, collector/scheduler health, marine/cloud/nowcast features, backfill plans/runs, calibration summaries, DB health/backup-operation cues, monitoring alert summaries and daily-report links, paper-live status, events, fusion/risk-guard notes, access posture, and ops posture. It is intended for loopback or carefully reviewed private access, not public exposure.

## Testing and validation status

Current local validation run: `PYTHONPATH=src python -m pytest tests -q` passed with **119 tests** on 2026-07-14.
